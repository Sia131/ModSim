...
for k = 1:length(p)
    ...
    %T = T + 0.5*M*norm(dp{k})^2; % add kinetic energy of mass k
    T = T + 0.5*M*dq'*dp{k}'*dp{k}*dq;
end
T = simplify(T);
...
for k = 1:length(p)
    V = V + M*g*k*L + M*g*p{k}(2); % add potential energy of mass k
end
V = simplify(V);
...
matlabFunction(p{1},p{2}, 'file','PendulumPosition','vars',{state, L});
